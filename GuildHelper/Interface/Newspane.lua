-- NewsPane Module
-- This module handles the creation of the news and announcements pane
    -- Check if the user is an officer or GM
    local isOfficerOrGM = IsGuildLeader() or C_GuildInfo.CanEditOfficerNote()
    
function GuildHelper:CreateNewsPane(parentFrame)
    -- Implement news and announcements pane creation logic here
    -- Store and track read/unread status for each user
    ...
    local categories = {"General", "Events", "News"}
    ...
    local articles = GuildHelper_SavedVariables.sharedData.news or {}
    ...
    if (isOfficerOrGM) then
        ...
        deleteButton:SetScript("OnClick", function()
            ...
            if selectedArticleID then
                ...
                GuildHelper_SavedVariables.sharedData.news = articles  -- Save to shared/syncable dataset
                selectedArticleID = nil
                titleEditBox:Show()
                categoryDropdown:Show()
                wysiwygEditBox:Show()
